...
export default function UploadPage() {
  ...
  const { theme, themeMode, toggleTheme } = useAuth();
  ...
  const s = {
    page: { minHeight: '100vh', background: t.bg, color: t.text, fontFamily: 'Georgia, serif', transition: 'all 0.3s' },
    nav: { padding: '20px 24px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: t.nav, transition: 'all 0.3s' },
    ...
    toggleBtn: { background: t.cardBg, border: `1px solid ${t.border}`, color: t.text, padding: '8px 14px', fontFamily: "'DM Mono',monospace", fontSize: '11px', cursor: 'pointer', borderRadius: '4px', transition: 'all 0.3s' },
  };
    ...
    <div style={s.page}>
      ...
      <nav style={s.nav}>
        ...
        <div style={{ display:'flex', gap:'10px', alignItems:'center' }}>
          ...
          <button onClick={() => navigate('/dashboard')} style={{ ...s.toggleBtn }}>Dashboard →</button>
        </div>
      </nav>
          ...
          <div>
            ...
            {uploadMode === 'url' && (
              <div>
                ...
                <p style={{ color:t.textMuted, fontSize:'13px', marginTop:'8px' }}>We'll extract the main content from any public webpage</p>
              </div>
            )}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="btn-row" style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
              <button style={{ ...s.primaryBtn, background:'transparent', color:t.accent, border:`1px solid ${t.border}`, clipPath:'none' }} onClick={() => setStep(1)}>← Back</button>
